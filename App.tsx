...
const AppContent = () => {
  ...
  const markAllAsRead = async () => {
    if (!user) return;
    const updates: any = {};
    updates[`notifications/${user.uid}`] = null;
      ...
      <main className="lg:ml-72 flex-1 relative pb-32 lg:pb-0 w-full flex flex-col">
        ...
        <header className="sticky top-0 z-[60] bg-white/80 backdrop-blur-xl border-b border-gray-100 px-4 md:px-10 py-3 md:py-4 flex justify-between items-center h-[72px] md:h-[88px]">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 md:gap-4 group">
              <div onClick={handleLogoClick} className="cursor-pointer select-none p-1 transition-transform group-hover:scale-110">
                <img src={LOGO_ICON_URL} className={`w-12 h-12 md:w-16 md:h-16 object-contain ${isLogoSpinning ? 'animate-spin-once' : ''}`} alt="Hotel Logo" />
              </div>
          ...
          <div className="flex items-center gap-3">
            {hasPendingBooking && (
              <div className="hidden lg:flex items-center gap-2 bg-amber-50 text-amber-600 px-4 py-1.5 rounded-full border border-amber-100">
                 <ShieldAlert size={14} />
                 <span className="text-[9px] font-black uppercase tracking-widest">Reviewing Stay</span>
              </div>
            )}
                  ...
                  <div className="absolute top-16 right-0 w-[300px] bg-white rounded-[2rem] shadow-2xl border border-gray-100 overflow-hidden animate-fade-in z-[2000]">
                    ...
                    <div className="max-h-[350px] overflow-y-auto no-scrollbar p-4 space-y-3">
                      {notifications.length > 0 ? (
                        ...
                        <div className="py-12 text-center text-gray-400 font-black uppercase tracking-widest text-[10px]">Registry Quiet</div>
                      )}
                    </div>
                  </div>
                )}
        ...
        <div className="flex-1">
          <Routes>
            <Route path="/" element={
              <article className="animate-fade-in">
                ...
                <section aria-label="Exclusive Offers">
                  <ExclusiveOffers offers={validOffers} isEditMode={isEditMode} claimedOfferId={claimedOfferId} onClaim={handleClaimOffer} onUpdate={(o) => setSiteConfig(prev => ({...prev, offers: o}))} onImageUpload={(f) => uploadToR2(f, 'offers')} />
                </section>
                <section aria-label="Available Rooms">
                  <RoomGrid rooms={siteConfig.rooms} isEditMode={isEditMode} activeDiscount={activeDiscount} isBookingDisabled={hasPendingBooking} onBook={handleRoomBookingInit} onUpdate={(r) => setSiteConfig(prev => ({...prev, rooms: r}))} onImageUpload={(f) => uploadToR2(f, 'rooms')} />
                </section>
            ...
            <Route path="/rooms" element={<RoomGrid rooms={siteConfig.rooms} activeDiscount={activeDiscount} isBookingDisabled={hasPendingBooking} onBook={handleRoomBookingInit} isEditMode={isEditMode} onUpdate={(r) => setSiteConfig(prev => ({...prev, rooms: r}))} onImageUpload={(f) => uploadToR2(f, 'rooms')} />} />
            ...
            <Route path="/admin" element={(isAdmin || isOwner) ? <AdminDashboard /> : <div className="p-20 text-center min-h-screen font-black text-gray-400 uppercase text-[10px] tracking-widest">Unauthorized Access</div>} />
          </Routes>
        </div>
        ...
        <footer className="bg-white border-t border-gray-100 py-20 px-6 md:px-12">
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="p-1.5 flex items-center justify-center">
                    <img src={LOGO_ICON_URL} className="w-14 h-14 object-contain" alt="Hotel Logo" />
                  </div>
                  <div>
                    <p className="text-[12px] font-bold text-gray-900 uppercase tracking-[0.2em]">Hotel Shotabdi</p>
                    <p className="text-[10px] text-gray-400 font-medium uppercase tracking-[0.1em] mt-0.5">Residential Hub</p>
                  </div>
                </div>
                <p className="text-[11px] text-gray-500 font-normal leading-relaxed max-w-xs">
                  Elite hospitality and residential perfection at the heart of Sylhet. Verified and ready for your arrival.
                </p>
              </div>
              ...
              <div className="space-y-6">
                 <h4 className="text-xs font-semibold text-gray-900 uppercase tracking-widest">Registry Contact</h4>
