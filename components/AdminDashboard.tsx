    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto pb-32 lg:pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-2">Residential Authority</p>
        </div>
        <div className="flex bg-gray-50 p-1.5 rounded-2xl border border-gray-100">
          {['bookings', 'users'].map((tab) => (
            <button key={tab} onClick={() => { setActiveTab(tab as any); setSearchQuery(''); }} className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white shadow-sm text-hotel-primary' : 'text-gray-400'}`}>
      ...
      <div className="relative mb-10 group">
        ...
        <input type="text" placeholder={`Search ${activeTab}...`} className="w-full bg-white border border-gray-100 rounded-3xl py-5 pl-14 pr-6 text-sm font-semibold outline-none focus:border-hotel-primary shadow-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
      </div>
        ...
        {activeTab === 'users' ? (
            ...
            {filteredUsers.map(user => (
              <div key={user.uid} className="bg-white rounded-3xl border border-gray-100 p-5 flex items-center justify-between hover:shadow-xl group">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl overflow-hidden border border-gray-100"><img src={user.photoURL} className="w-full h-full object-cover" /></div>
                  <div>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setSelectedUser(user)} className="p-3 text-gray-300 hover:text-hotel-primary"><Eye size={18} /></button>
                  <button onClick={() => setUserToDelete(user)} className="p-3 text-gray-300 hover:text-red-600"><Trash2 size={18} /></button>
                </div>
              </div>
            ))}
          </div>
          ...
          filteredBookings.map(booking => (
            <div key={booking.id} className={`bg-white rounded-3xl border border-gray-100 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 ${booking.status === 'pending' ? 'ring-1 ring-hotel-primary/10 shadow-lg' : 'opacity-80'}`}>
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${booking.status === 'pending' ? 'bg-hotel-primary text-white' : (booking.status === 'rejected' ? 'bg-red-50 text-red-400' : 'bg-green-50 text-green-400')}`}>
                  <Building2 size={20} />
                </div>
                <div>
                  ...
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{booking.userName} • {booking.totalGuests} Pers.</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                 ...
                 <div className="flex items-center gap-2">
                    {booking.status === 'pending' ? (
                      <>
                        <button onClick={() => setRejectingBookingId(booking.id)} className="p-3 text-gray-300 hover:text-red-600"><XCircle size={22} /></button>
                        <button onClick={() => setAcceptingBookingId(booking.id)} className="bg-hotel-primary text-white px-6 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-lg shadow-red-100">Approve</button>
                      </>
                    ) : (
                      <span className={`px-4 py-2 rounded-xl text-[8px] font-black uppercase tracking-widest flex items-center gap-2 ${booking.status === 'accepted' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                        {booking.status === 'accepted' ? <CheckCircle2 size={12}/> : <ShieldAlert size={12}/>}
                        {booking.status}
                      </span>
                    )}
                 </div>
      ...
      {acceptingBookingId && (
        <div className="fixed inset-0 z-[300] bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl">
            ...
            <p className="text-[9px] font-bold text-gray-400 uppercase mb-8 text-center">Confirm assignment</p>
            ...
            <div className="flex gap-4">
              ...
              <button disabled={!roomNumberInput} onClick={() => { const b = bookings.find(x => x.id === acceptingBookingId); if (b) handleBookingAction(b, 'accepted', roomNumberInput); }} className="flex-1 bg-hotel-primary text-white py-4 rounded-2xl font-black text-[9px] uppercase shadow-lg shadow-red-100">Assign</button>
            </div>
          </div>
        </div>
      )}
      ...
      {rejectingBookingId && (
        <div className="fixed inset-0 z-[300] bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl">
            <h2 className="text-2xl font-serif font-black mb-1">Decline Stay</h2>
            <p className="text-[9px] font-bold text-gray-400 uppercase mb-8 text-center">Select reason</p>
            <select className="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 mb-6 text-xs font-bold outline-none" value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)}>
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => { const b = bookings.find(x => x.id === rejectingBookingId); if (b) handleBookingAction(b, 'rejected', rejectionReason); }} className="flex-1 bg-red-600 text-white py-4 rounded-2xl font-black text-[9px] uppercase shadow-lg">Confirm Rejection</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[8px] font-black text-hotel-primary uppercase">Identity Record</p>
                </div>
              </div>
            ...
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div><p className="text-[8px] font-black text-gray-400 uppercase mb-1">Age</p><p className="text-xs font-black">{selectedUser.age || 'N/A'}</p></div>
              </div>
              <div className="rounded-[2rem] overflow-hidden border-2 border-gray-100 aspect-video bg-gray-100 shadow-inner">
